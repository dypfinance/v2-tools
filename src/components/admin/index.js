...
export default class Admin extends React.Component {
    ...
    handleFarmFileUpload = async (e) => {
        ...
        let signature = await window.sign(window.config.metamask_message, await window.getCoinbase())
    ...
    handleProjectFileUpload = async (e) => {
        ...
        json = json.map(item => {
            ...
            item.ts_score_avg = (item.ts_score_security + item.ts_score_information + item.ts_score_liquidity + item.ts_score_tokenomics) / 4
        ...
        for (let item of json) {
            ...
            let pair_ids = item.pair_ids.split(',').map(i => i.trim().toLowerCase())
    ...
    GetAdminForm = () => {
        return (
            <div>
                <strong style={{fontSize: '1.2rem'}} className='d-block mb-3'>UPDATE PAIR INFORMATION</strong>
                <form style={{maxWidth: '600px'}} onSubmit={this.handleSubscribe}>
                    <p className='text-muted mb-4' style={{fontSize: '.8rem'}}><i className='fas fa-info-circle'></i> This will authenticate admin and update project information including trust scores and links related to projects.</p>
                    ...
                    <br /> */}
                    {this.props.appState.isConnected ? <div>
                        

    

                        <label style={{minWidth: '250px'}} disabled={!this.props.appState.isConnected} className='btn v1 p-2' type='submit'>
                            UPLOAD PROJECT CSV
                            <input className='d-none' type='file' onChange={this.handleProjectFileUpload} accept='text/csv' />
                        </label>
                        <br />
                        <br />
                        <label style={{minWidth: '250px'}} disabled={!this.props.appState.isConnected} className='btn v1 p-2' type='submit'>
                            UPLOAD FARM CSV
                            <input className='d-none' type='file' onChange={this.handleFarmFileUpload} accept='text/csv' />
                        </label>
                    </div>
                    :
                    <div>
                        ...
                        <button disabled={this.props.appState.isConnected} onClick={this.props.handleConnection} className='btn v1' type='button'>CONNECT WALLET</button>    
                    </div>}
                </form>
    ...
    render() {
        ...
        return (
            <div className='locker'>
                <h2 style={{display: 'block', color: `var(--preloader-clr)`}}>Admin, Pair Info Update</h2>
